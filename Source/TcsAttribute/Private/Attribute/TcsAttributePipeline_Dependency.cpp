...
namespace
{
	...
	void StrongConnect(
		const FGameplayTag& Node,
		const TMap<FGameplayTag, TArray<FGameplayTag>>& Graph,
		TMap<FGameplayTag, int32>& Index,
		TMap<FGameplayTag, int32>& LowLink,
		TArray<FGameplayTag>& Stack,
		TSet<FGameplayTag>& OnStack,
		int32& NextIndex,
		bool& bOutHasCycle)
	{
		...
		if (const TArray<FGameplayTag>* Readers = Graph.Find(Node))
		{
			for (const FGameplayTag& Next : *Readers)
			{
	...
	// 环检测（严格模式）：图中存在环 → true
	bool HasDependencyCycle(const TMap<FGameplayTag, TArray<FGameplayTag>>& Graph)
	{
		TMap<FGameplayTag, int32> Index;
		TMap<FGameplayTag, int32> LowLink;
		TArray<FGameplayTag> Stack;
		...
		for (const TPair<FGameplayTag, TArray<FGameplayTag>>& Pair : Graph)
		{
...
void FTcsAttributePipeline::RegisterDependency(
	FTcsAttributeStore& Store, const FGameplayTag& ReadAttribute, const FGameplayTag& ReaderAttribute)
{
	// 自引用已在属性添加期拒绝（D2-4）；此处静默跳过（防御）
	if (ReadAttribute == ReaderAttribute || !ReadAttribute.IsValid())
	{
		return;
	}

	TArray<FGameplayTag>& Readers = Store.Dependents.FindOrAdd(ReadAttribute);
	...
	if (HasDependencyCycle(Store.Dependents))
	{
		...
		ensureMsgf(false,
			...
			*ReaderAttribute.GetTagName().ToString(), *ReadAttribute.GetTagName().ToString());
	}
}

void FTcsAttributePipeline::MarkDependentsDirty(
	FTcsAttributeStore& Store, const FGameplayTag& ChangedAttribute)
{
	const TArray<FGameplayTag>* Readers = Store.Dependents.Find(ChangedAttribute);
	if (!Readers)
	{
		return;
	}
	...
	for (const FGameplayTag& Reader : *Readers)
	{
