...
- **Def identity & primary-asset standard (2026-09-17)**: each definition has exactly **one id per track** — a DataTable row's identity is its **RowName** (no `DefId`/`TemplateId` field inside the row struct), and an asset's identity is its **`[PrimaryAssetType, DefId]`**: Def assets declare `static const FPrimaryAssetType PrimaryAssetType` explicitly (not derived from the class name) and override `GetPrimaryAssetId()` so the *name* comes from `DefId`/`TemplateId` rather than the asset's own name — asset files may be renamed or moved without breaking resolution. Registering Definitions with a consumer therefore passes the id explicitly (the row carries none). Registering the types in `PrimaryAssetTypesToScan` belongs to the M6 DefLibrary round (ids resolve without it).
