...
当前 change 最初是按“保留 `UTcsStateTreeSchema_StateInstance` 作为最小 concrete schema”来推进的，但现在架构方向已经进一步收敛：
...
2. `UTcsStateTreeSchema_StateInstance` 继续存在，会把抽象执行基类重新暴露成 concrete editor-facing schema。
...
- 目标：删除 `UTcsStateTreeSchema_StateInstance`，不再保留 generic concrete `StateInstance` schema。
...
- 目标：继续保留 `UTcsStateSchema_StateComponent` 的 `LinkSubTree` / `LinkedSubTree` 兼容方向。
...
- 决策：删除 `UTcsStateTreeSchema_StateInstance`，且不再引入任何 concrete `UTcsStateSchema_StateInstance` 替代物。
...
- 决策：组件树 schema 命名统一为 `UTcsStateSchema_StateComponent`。
...
- 决策：Buff 树 schema 命名统一为 `UTcsStateSchema_Buff`。
...
- 决策：Skill 树 schema 命名统一为 `UTcsStateSchema_Skill`，根上下文名同步收敛为 `SkillInstance + SkillEntry`。
...
- 风险：删除 `UTcsStateTreeSchema_StateInstance` 后，旧资产和旧 editor authoring 入口会失去直接迁移目标。
...
- 风险：`UTcsStateSchema_StateComponent` 继续走组件 schema 兼容路线，容易让人误解它就是“普通 state 的替代 concrete 线”。
