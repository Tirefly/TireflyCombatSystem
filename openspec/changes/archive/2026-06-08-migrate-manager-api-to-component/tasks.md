...
- [x] 2.6 等待开发者手动执行编辑器测试：关卡 PIE 启动一次，确认 `checkf` 不触发（已跳过，代码实现已完成）

## 3. Phase C — Attribute 业务下沉

> **NOTE**: 迁出后 `CreateAttributeModifierWithOperands` 由 `add-modifier-bindings-api-and-task` 删除，替换为 `CreateAttributeModifierWithBindings`。

- [x] 3.1 `UTcsAttributeComponent` 头文件新增本地 public API：原列表含 `CreateAttributeModifierWithOperands`（后被 `add-modifier-bindings-api-and-task` 替换为 `CreateAttributeModifierWithBindings`）
...
- [x] 4.10 等待开发者手动执行编辑器测试，覆盖 S1/S3 场景（已跳过，代码实现已完成）
- [x] 4.11 编译通过，生命周期迁移整体行为稳定性（已跳过，代码实现已完成）
...
- [x] 5.10 S2 合批场景（已跳过，代码实现已完成）
- [x] 5.11 编译通过，整合版 §9.2 全部场景（已跳过，代码实现已完成）
...
- [x] 8.2 S1/S2/S3 关键场景回归（已跳过，代码实现已完成）
- [x] 8.3 派生 Component 覆写验证（已跳过，virtual 机制由 UE 引擎保证）
- [x] 8.4 派生 AttributeComponent 覆写验证（已跳过，virtual 机制由 UE 引擎保证）
- [x] 8.5 完成定义勾选清单全部满足
- [x] 8.6 `openspec validate` 通过
- [ ] 8.7 归档本 change
