...
- **WHEN** `SkillDef` 变为可直接资产化的类型
- **THEN** `UTcsSkillDefinition` 已通过 `UTcsSkillDefinitionFactory` + `UAssetDefinition_TcsSkillDefinition` 归入 `Tirefly Combat System -> Definition Asset` 菜单
- **AND** `UTcsSkillModifierDefinition` 已同步纳入 editor authoring 与 PrimaryAssetType 勘误覆盖
- **AND** Skill 创作流程未拆分成无关的编辑器创作路径
