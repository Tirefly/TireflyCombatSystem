...
- 字段：`FName FlowTemplateId`（**空 = 官方默认模板**）、`FTcsParamValue DamageBase`（基础伤害值输入——参数账本解算结果，PV-7）、`TMap<FName, FTcsParamValue> FormulaParams`（公式参数初值）；
...
- 执行器 MUST：构 `FTcsDamageFlowContext`（`Attacker`/`Instigator` 取 `Context.Caster`/`Instigator` 句柄、`Targets` 取 `Context.Targets`——**消费目标集，不内嵌选择器**，D4-4 v2）→ 把 `DamageBase` 与 `FormulaParams` 写入上下文（黑板键 `BaseDamage` / 公式参数表）→ `RunTemplate` → 恒返回 `TSR_Completed`（流程单帧同步完成，无挂起）；
...
- **THEN** 流程按模板跑完，目标的 `HealthAttrKey` 属性被事务扣减，记录事件已发布
