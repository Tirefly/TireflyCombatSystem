...
- [x] 1.4 让快照发现覆盖 AttributeDef、AttributeModifierDef、StateDef 和 StateSlotDef 资产。
- [x] 1.5 让快照发现接受基于对应 TCS 基类派生的资产。
...
- [x] 2.2 验证在编辑器中新建 StateDef 或 StateSlotDef 时，registry 快照、`UTcsDeveloperSettings` 兼容视图以及 State manager subsystem 都会在不重启编辑器的前提下更新。
- [x] 2.3 验证在同一编辑器会话中，修改、重命名和删除现有 Def 资产也会被正确传播。
- [x] 2.4 验证基于 TCS 基类派生出来的 Def 资产仍然能被 registry 发现并建立索引。
