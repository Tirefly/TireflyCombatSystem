...
>
> 第 8 阶段执行状态：已改为小阶段 review 模式。未通过编译与实际 Automation 验证的内容不得勾选为完成。
...
## 6. Skill DefId 主路径
- [x] [AI] 6.1 为 `UTcsSkillEntry` 明确“对外使用 `SkillDefId`，实例内部缓存已校验 `UTcsSkillDefinition*`”的权威模型。
- [x] [AI] 6.2 将 `LearnSkill` / `ActivateSkill` 主路径改为按 `SkillDefId` 驱动。
- [x] [AI] 6.3 直接移除对象指针版本的 public 入口；若迁移阶段存在残余逻辑，只允许退化为内部辅助转发实现。
- [x] [AI] 6.4 明确 SkillDef 解析失败、缺失或加载失败时的运行时错误语义。

## 7. DefId 主路径扩展到 Buff / SkillModifier
- [x] [AI] 7.1 将 Buff apply 主路径收敛到 `StateDefId` / `BuffDefId` 驱动，不要求调用方先持有已加载 Def 对象。
- [x] [AI] 7.2 将 SkillModifier apply 主路径收敛到 `SkillModifierDefId` 驱动，不要求调用方先持有已加载 Def 对象。
