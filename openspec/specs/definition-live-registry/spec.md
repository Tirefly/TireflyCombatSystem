...
## Purpose
定义 `UTcsDefinitionRegistrySubsystem` 如何作为编辑器期 Def 的权威快照持有者，持续维护 Attribute、AttributeModifier、state-side DefinitionAsset（通过抽象 `UTcsStateDefinition` 基类，包含具体 `UTcsBuffDefinition`）以及 StateSlot 的实时 registry，并将兼容缓存镜像到 `UTcsDeveloperSettings`，同时让管理子系统与最新快照保持同步。
## Requirements
