...
| **D2-1 修订（2026-09-22）** | **属性身份载体：FName 包装 → `FGameplayTag`**——原 D2-1"FName 词表 + 显式包装结构"被**有据重开**（提案 `switch-identifiers-to-gameplay-tags`）。当年否决 tag 的三条理由逐条核实：①"无编译期检查"——FName 同样没有，**平手**（且 tag 的 `protected` 构造 + `RequestGameplayTag` 的 ensure 更强）；②"字符串比较开销"——`FGameplayTag` 内部即 `FName`，`GetTypeHash` 逐字相同，**平手**；③"易拼错"——tag 反超（编辑器 picker + `GameplayTagsRedirects` 重命名自动修引用 + 构造处 ensure）。**上表 D2-9 的"权威=DefId"应读作"权威=`DefTag`"**（引用规范本身不变，只是身份载体改型；"FAttributeName 稠密缓存覆盖词表"一条随包装删除而作废——tag 自身承载身份）。 | 已拍板 2026-09-22 |

| D2-16 | **覆盖带强弱口径终定**（2026-09-18 拍板，落地于 plan1 Task 5）：三级阶梯——①`OverridePriority`（**修正器侧**，大者胜，唯一第一裁决键）→ ②`OverrideTieBreak`（**属性定义侧**，封闭四值：取最大/取最小/绝对值最大/绝对值最小）→ ③有符号值（补齐全序）。**框架不定义任何其它"谁盖谁"的规则**；默认值 = 历史行为（全 0 + 取最大 ≡ 旧的"组内最大值"）。动因：数值本身不含方向——"取最大值"对护甲（越大越强）对，对承伤倍率/冷却（越小越强）会取到**最温和**的一条，同一套规则不可能两边都对 | 已拍板 2026-09-18 |

**D2-16 的被否方案与理由（决策留痕，避免后议重来）**：

| 被否方案 | 否决理由 |
|---|---|
| 属性定义加"方向字段"（越高越强 / 越低越强），Override 按方向取 max 或 min | 只能翻 max/min 一根轴，**表达不了"数值更小的反而该赢"**（变身 B 护甲 30 覆盖 A 的 50）；且要把"方向"概念下推给每个折叠器调用方 |
| 按绝对值最大比较（保留符号） | 一行改动、全正全负都对；但正负混合时语义变成"谁幅度大谁赢"，规则读代码看不出来（有魔法），且无法显式排座次 |
| 操作词表加 `TAO_OverrideLow`（组内取最小） | 不动签名，但两条"绝对陈述"并存时要另定优先级——把方向问题换成优先级问题，只是挪了困惑 |
| 框架给"按来源分层"的优先级建议（装备 0 / buff 10 / 变身 100） | 用户口径：属**二次规则**，"到底以哪个为准"对使用者有争议——协调完全交给 `OverridePriority` |
| 开放 CustomStrategy（自定义同优先级裁决） | 热路径比较函数必须**全域且确定**（赢家与遍历顺序无关）；自定义会把"谁说了算"变回不可静态推演（与 `AVD_Custom` 不同源：后者是值域语义且有 Clamp 回落） |
