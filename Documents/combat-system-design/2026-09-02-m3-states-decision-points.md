...
| **身份载体修订（2026-09-22）** | **本文档所有 `DefId` 应读作 `DefTag: FGameplayTag`**（D3-18 实例权威、D3-19 `ModifierRows{TemplateId}` → `{TemplateTag}`、`ApplyState` 签名、`FStateInstance` 身份字段等）——原 `FName` 身份载体被**有据重开**（提案 `switch-identifiers-to-gameplay-tags`；三条理由逐条核实见 m0min-m2 文档的 D2-1 修订行）。**引用规范本身不变**（权威=身份、热路径解析缓存、运行期零回查），只是身份载体由 `FName` 改 tag。 | 已拍板 2026-09-22 |
