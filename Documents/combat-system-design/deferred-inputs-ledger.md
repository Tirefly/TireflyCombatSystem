...
> **背景**：LAC 已接入 UnrealSharp（上游 main @ `46e9c2c1`，UE 5.8，`Script/LegendAutoChessCS` 实测热重载通过）。用户 2026-09-24 拍板**脚本化的三个动机都有**（摆脱 C++ 编译 / 摆脱蓝图低扩展性 / 写全新语义），**但不排斥 C++**——"非常适合放在 TCS 插件基建或 LAC C++ 底层的，肯定没问题"。
> **判据来源**：`2026-09-23-csharp-tcs-logic-authoring-research.md`（**glue 产物实证**——直接读 TCS 已生成的 76 个 `.generated.cs`，非源码推断）。
> **与 G-3/G-4 的关系**：G-3（效果链 12/15 原语缺口）**已在 R5-2 登记**，不重复；G-4（4 个策略基类虚分派）为可选架构改造、用户未拍板，不入册。
> **分层结论**（本次实证）：数据面 ✅ 通（C# 可定义纯数据 USTRUCT、可组装 `IList<T>` 送进链/模板）；行为登记面 ❌ 不通；门面调用面 ❌ 不通。

| # | 事项 | 来源锚点 | 现状证据（已核） | 状态 |
|---|---|---|---|---|
| S-1 | **门面 API 反射化**（A 类加 `UFUNCTION` + B 类形参反射化）：**A 类** = `RegisterChain` / `RegisterTriggerRow` / `RegisterTemplate` / `SetEntityQuery` / `SetTriggerGateTag` / `GetTriggerRowCount`（形参已全反射、glue 已在，**加宏即可**）；**B 类** = `FTcsChainRunHandle` / `FTcsSourceHandle` / `FTcsAttrModInstance` / `FTcsEffectContext` 反射化（现均**无 `USTRUCT` 宏、无 glue**），或改门面签名为 tag/句柄形态。**A/B 必须同批**——只做 A 会得到"能注册链但发起不了链"的半通面（`ExecuteChainById` 返回 `FTcsChainRunHandle`） | `effect-step-dispatch/spec.md:12`（"注册入口 MUST 双份…**反射层 / 脚本层 / 测试装置可达**"）；`04-module-effects.md:39`/`:56`（D4-14/17"反射可达动态委托入口"）；`2026-09-23-csharp-tcs-logic-authoring-research.md` §4.3；**用户 2026-09-24 拍板为下一步** | 实测：`TcsEffect/TcsEffectSubsystem.generated.cs` **只有两个属性、零方法**（`RegisterChain` / `ExecuteChain` / `SetEntityQuery` 等全未导出）；B 类三 struct 无宏无 glue（2026-09-24 逐处核） | **✅ 已消费（2026-09-24，提案 `add-scripting-reflection-surface`）**——落点：`UTcsEffectSubsystem`（链/触发/执行侧 10 方法 + 新增 `IsChainRegistered` / `ExecuteChainForCaster`）、`UTcsAttributeSubsystem`（实体注册面 6 方法）、`UTcsCombatEntityComponent`（`ExecuteChainById`）；**两个句柄展平为可反射字段**（`FTcsChainRunHandle` / `FTcsEffectTriggerHandle`）。**PIE 实测通过**：脚本层注册实体 → 注册链 → 起链 → 接住句柄（`Index=0 Generation=1`）→ 传回 `IsRunActive` 返 `true` → 链被到期堆唤醒跑完。**未覆盖（留待后续）**：`UnregisterTriggerRowsBySource`（形参 `FTcsSourceHandle` 非反射）、`SetEntityQuery`/`GetEntityQuery`（依赖 `ITcsEntityQuery` 反射化，见 S-5）、`FindChain`（裸 struct 指针不可反射） |
| S-2 | **三张注册表的反射注册入口**（C 类换签名）：`FTcsEffectStepExecutorRegistry::Register` / `FTcsTriggerConditionRegistry::Register` / `FTcsTriggerPayloadReaderRegistry::Register` —— `TFunction` 形参 → 反射可见委托（`BlueprintNativeEvent` 或 `FScriptDelegate`）；三处**同批**（口径一致，任一处单独开则三处不一） | `TcsTriggerCondition.h:132-142` 与 `TcsTriggerPayloadReader.h:118-128` 注释**自认欠账**（"该欠账与步骤执行器注册表**同批**解决（CS 调研 §7.6 的 G-2）"）；`TcsEffectStepExecutor.h:70-76`（D4-17 双入口之反射面：脚本层 / 测试装置 / 运行期补登）；`effect-trigger/spec.md:47`（"反射面欠账（明示，非遗漏）"）；CS 调研 §7.6 G-2 | 未落地：三处 `Register` 均无 `UFUNCTION`、形参含 `TFunction`；TCS 全模块 `UFUNCTION` 仅 7 处、**无一处通向注册表**（2026-09-24 复核） | 待触发（**签名须按 S-3 的上下文形态定**——"什么能被脚本提供"取决于上下文反射化方案） |
| S-3 | **上下文 / 运行态反射化（G-1）**：①`FTcsEffectContext`（字段全反射：句柄×2 + `FInstancedStruct` + `TArray<句柄>` + `TMap<tag,double>`——**可做，但 `effect-chain/spec.md:63` 明文"MUST NOT 为反射类型"，须同批修订或改走"反射镜像"路线**）；②`FTcsChainRun`（含 `TWeakObjectPtr` + 裸模板 `TTcsInstanceHandle` + `FTcsTimeEntryHandle`——第二道坎，包 USTRUCT 可解，先例 = `FTcsCombatEntityHandle`）；③**`FTcsDamageFlowContext` 物理不可直接反射化**——黑板 `FTcsFlowAttributes` 深处嵌 `FTcsConsumePolicy::OnConsumed`（`TFunction<void()>`）⇒ **真前置 = 上下文 / 黑板分层**（把含闭包的运行态面从数据面摘出，或给反射镜像） | `effect-chain/spec.md:63`（"结构 MUST NOT 为反射类型（纯运行态，非配置数据）"——**与脚本化需求直接冲突**）；`param-value/spec.md:46-48`（"MUST 是反射可见 USTRUCT——禁止 TFunction"——同族纪律的另一面）；`TcsFlowDataSteps.h:44-46`（TCS 自认"含 `TFunction` 回调…不可反射、不可作 UPROPERTY——UHT 实证"）；CS 调研 §5 | 未落地；**物理约束已实证**（递归闭包 `FTcsDamageFlowContext` → `FTcsFlowAttributes` → `FTcsFlowAttributeSubmit` → `FTcsConsumePolicy` → `TFunction`，2026-09-24 亲读源码） | 待设计（**须先出"上下文 / 黑板分层"设计提案**——用户 2026-09-24 接受该顺序：提案但不动手） |
| S-4 | **`ITcsDamageFlowDelegate` 反射化（自定义伤害流程出口）**：5 个方法加 `UFUNCTION(BlueprintNativeEvent)` + C++ 调用点改 `Execute_*`（`GetBaseHitRate` / `GetBaseCritRate` / `ResolveElement` / `CalculateBaseDamage` / `ModifyShield`） | `09-module-damage.md` §2.4（`ICombatDamageFlowDelegate` 纯 C++ 接口、宿主实现）与 §2.2（"自定义步骤的 C++ 边界"：新公式归 delegate）；CS 调研 §6；**先例 = `ITcsAttributeProvider`**（同代码库内现成对照样本） | 未落地：5 方法零 `UFUNCTION`；C++ 调用点是**虚表直调**（`TcsFlowStepsRest.cpp:80/106/130/206`、`TcsFlowStepsCore.cpp:103/148`）⇒ **C# wrapper 是空壳**（`TcsDamageFlowDelegate.generated.cs` 实测）——C# 类"实现"它编译期不报错，但 C++ 虚表调用永远到不了 C# | 待触发（**形参含 `FTcsDamageFlowContext` ⇒ 依赖 S-3 的分层结论**；流程侧 12 个执行器已全实现，是四个脚本化方向里基础设施最完整的） |
| S-5 | **`ITcsEntityQuery` 反射化（脚本读世界）**：找敌人 / 取位置 / 判存活——形参含 `TFunctionRef<void(FTcsCombatEntityHandle)>`（C 类），须换签名 | `entity-query-contract` 规格；`TcsEntityQuery.h:32`（自注"C++ 专用面：参数含 `TFunctionRef`，蓝图不可表达"）；CS 调研 §6.3 | 未落地：三方法零 `UFUNCTION`；glue wrapper 空壳（`TcsEntityQuery.generated.cs` 实测）；宿主唯一实现 `UTcsPieEntityQuery` 是 C++ 类 | 待触发（**无此能力则 C# 只能"改自己"、无法"选目标"**——`SelectTargets` 步骤的策略位同样不可达） |
| S-6 | **`TInstancedStruct<T>` 字段的 C# 导出缺口**（2026-09-24 实测发现，比 S-1 更靠前）：UnrealSharp 的 `PropertyTranslatorManager` **未为 `UhtTemplateStructProperty` 注册 translator**（全库零命中），而 `GetTranslator` 按**精确类型**查表 ⇒ `TInstancedStruct<T>` 字段被跳过 ⇒ 含策略位的 struct 在 C# 侧是**空壳**。受影响：`FTcsParamValue.Source`（**全插件数值载体**——链步骤数值/修正器 Operand/流程 Operand 全走它）、`FTcsStepSelectTargets.Selector`/`Filters`、`FTcsAttrModInstance`、未来 R5/R6 的 `ApplyState`/`ModifyAttribute` | 用户 2026-09-24 实测后质询"为什么不能直接使用 FInstancedStruct"；`2026-09-23-csharp-tcs-logic-authoring-research.md` §1；**提案 `add-scripting-reflection-surface` 的实证边界即此缺口**（三步骤链因 `SelectTargets` 空壳而改用 Damage+WaitDelay；`FTcsStepDamage.DamageBase` 配不了 ⇒ 实测伤害 Base=0.000） | 实测：`FTcsParamValue.generated.cs` / `FTcsStepSelectTargets.generated.cs` 均**零字段零构造参数**（空壳）；`FTcsParamSource_Literal` 等子类本身能导出（缺口在"模板字段"这一层，不在类型本身） | **✅ 已消费（2026-09-24，提案 `switch-strategy-carrier-to-plain-instanced-struct`）**——**换型为裸 `FInstancedStruct` + `meta = (BaseStruct = ...)`**（用户拍板方案）。落点：`FTcsParamValue.Source`（TcsCore）、`FTcsStepSelectTargets.Selector`/`Filters`（TcsTargeting）+ 5 处调用点适配 + 3 处头注释口径同步。**实测全部通过**：①glue 生成真实字段读写代码（非空壳）；②**资产兼容性 2/2 条、失败 0 条**（`Formula_Chain 步数=7` / `Slice_Chain 步数=3` 原样加载——三重论证：引擎保证同尺寸且"反射层视为同一物"、UHT 属性结构逐字段相同、资产序列化的是内层类型身份）；③**三步骤链补跑成功**（`SelectTargets[..]: 候选 1 → 通过 1`——换型前不可能）；④**脚本层配数值生效**（`DamageBase=25` → `Health：100.000 → 75.000`）。**代价（已接受）**：丢编译期类型限定（改由 `GetPtr` 运行期 `IsChildOf` 校验兜底）、picker 限定靠手写 metadata（写错则静默不限，建议入 M8 校验矩阵——归 R8-1） |
| S-7 | **脚本层正常竞态的红字噪音**（2026-09-24 实测发现，**已修**）：`UTcsEffectSubsystem::UnregisterChain` 的"有活动运行态"拒绝面原用 `ensureMsgf`——但脚本层"起一条挂起链后想注销"是**完全正常的调用序列**（时序竞态，非配置错误），产出红字噪音；且同函数内"未登记就注销"用的是 `Warning`，**两种拒绝面口径不一致** | 2026-09-24 PIE 实测（两次实测均出现该 ensure）；TCS 内部既有惯例：悬空句柄/陈旧句柄/世界拆解期均"正常竞态不 ensure"（`TcsEffectSubsystem.cpp:167`、`TcsTriggerRegistry.cpp:56`）；D0-6 日志纪律（红字留给真实缺陷） | 已修为 `UE_LOG(Warning)` + 返回 false | **✅ 已消费（2026-09-24，随提案 `add-scripting-reflection-surface`）**——本条登记为"口径统一的先例"，供后续新增门面 API 时参照：**配置错误 → ensure；时序竞态 → Warning** |
...
- **2026-09-24 增补（C# 脚本化可行性调研收束）**：**新开「脚本化工作流（S 系列）」区段**，登记 **S-1 ~ S-5** 五条——判据来源是 `2026-09-23-csharp-tcs-logic-authoring-research.md`，该文档**改用 glue 产物实证**（直接读 TCS 已生成的 76 个 `.generated.cs`）替代此前的源码推断。要点：①**S-1 门面 API 反射化**——分 A 类（加宏即可）/ B 类（形参非反射），**A/B 必须同批**否则得到"能注册但发起不了链"的半通面；②**S-2 三张注册表换签名**（C 类）；③**S-3 上下文反射化**——含**物理约束实证**（`FTcsDamageFlowContext` 深处嵌 `TFunction`，"整体反射化"不可能，真前置 = 上下文/黑板分层），且 `effect-chain/spec.md:63` 的"MUST NOT 为反射类型"与脚本化需求**直接冲突**须同批处理；④**S-4 伤害流程委托**（C# wrapper 空壳，"看着通实际不通"，先例 = `ITcsAttributeProvider`）；⑤**S-5 实体查询**（脚本读世界，此前未记入任何缺口清单）。**归属说明**：G-3（原语缺口）已在 R5-2，不重复；G-4（策略基类虚分派）为可选架构改造、用户未拍板，不入册。**用户 2026-09-24 拍板**：S-1 为下一步（含 `FTcsChainRunHandle` 反射化 + 三步骤链 C# 实证），S-3 先出设计提案不动手。条目总数 **27 → 32**。
  - **同批口径修正**：上一条（2026-09-23 Task 1 精修）末句写的"**新增待办**：②条件求值器与步骤执行器的**反射注册入口**（CS 调研 §7.6 G-2 的共享欠账）"——该"待办"此前**只有这句话、无表行**，本次已正式落为 **S-2**。
- **2026-09-24 增补（S-1 消费 + 实测发现）**：**S-1 ✅ 已消费**——提案 `add-scripting-reflection-surface` 落地（门面 API 反射化 + 两个句柄展平 + 实体注册面反射化），**PIE 实测通过**（脚本层注册实体→注册链→起链→接住句柄→传回查询返 true→链被唤醒跑完）。**新增 S-6 / S-7**：
  - **S-6（`TInstancedStruct<T>` 字段导出缺口）**——实测发现，**比 S-1 更靠前**：UnrealSharp 未注册 `UhtTemplateStructProperty` translator ⇒ 含策略位的 struct 在 C# 侧是空壳（`FTcsParamValue` / `FTcsStepSelectTargets` / `FTcsAttrModInstance` 等）。**用户 2026-09-24 拍板**：换型方案（裸 `FInstancedStruct` + `meta=(BaseStruct=...)`）单列提案下一轮做。
  - **S-7（脚本层正常竞态的红字噪音）**——**已修**：`UnregisterChain` 的"有活动运行态"拒绝面 `ensureMsgf` → `Warning`（时序竞态非配置错误；与同函数"未登记"口径统一）。登记为**口径先例**：配置错误 → ensure，时序竞态 → Warning。
  - **本轮实证的三条可复用教训**（写入 CS 调研文档 §11）：①**"能导出 ≠ 能往返"**——判类型能否被脚本往返使用，须检查绑定产物的 `ToNative`/`FromNative` 是否有真实字段读写代码（空壳能编译、能当类型用，但值过不去）；②**模板类型字段不可作 `UPROPERTY`** ⇒ 反射化句柄必须**展平**（先例 `FTcsCombatEntityHandle`）；③**门面反射面一打通，既有隐藏缺陷会集中暴露**（`FTcsEffectTriggerHandle` 的往返缺陷此前不可见，因 C# 根本调不到）。
  - 条目总数 **32 → 34**（S-1 消费不减条目、新增 S-6/S-7）。
- **2026-09-24 增补（S-6 消费：策略载体换型）**：**S-6 ✅ 已消费**——提案 `switch-strategy-carrier-to-plain-instanced-struct` 落地：`TInstancedStruct<T>` → **裸 `FInstancedStruct` + `meta = (BaseStruct = ...)`**（`FTcsParamValue.Source` / `FTcsStepSelectTargets.Selector`·`Filters` + 5 处调用点 + 3 处头注释）。**实测全绿**：glue 字段可读写（非空壳）、**资产兼容性 2/2 条失败 0 条**（两个含 `TInstancedStruct` 数据的资产原样加载）、**三步骤链补跑成功**（`SelectTargets[..]: 候选 1 → 通过 1`）、**脚本层配数值生效**（`DamageBase=25` → `Health 100 → 75`）。
  - **兼容性三重论证**（可复用判据，本次实测成立）：①引擎保证 `TInstancedStruct` 与 `FInstancedStruct` **同尺寸且"反射层视为同一物"**（`InstancedStruct.h:538`）；②UHT 产出的属性结构**逐字段相同**（`FStructProperty` → `FInstancedStruct`，`TcsParamValue.gen.cpp:70` 实证）；③资产序列化的是**内层类型身份**（`Ar << TObjectPtr<UScriptStruct>`，`InstancedStruct.cpp:191`），**与容器类型无关**。
  - **新增待办（建议入 M8 校验矩阵，归 R8-1）**：`BaseStruct` metadata **写错则 picker 静默不限**（`TryFindTypeSlow` 找不到返回 nullptr 不报错）——需"metadata 存在性与正确性"校验。
  - 条目总数仍 **34**（S-6 消费不减条目）。
