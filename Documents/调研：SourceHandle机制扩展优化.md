...
**删除字段说明**：
- `SourceDefinition (FDataTableRowHandle)` → 删除。各实例（Modifier、State、Skill）自身已持有定义引用（如 `FTcsAttributeModifierInstance::ModifierDefAsset`、`UTcsStateInstance::StateDefAsset`），SourceHandle 不应重复承担"标识来源定义"的职责
- `SourceName (FName)` → 删除。可从各实例自身的定义中获取，属于冗余字段

**新增字段说明**：
- `CausalityChain (TArray<FPrimaryAssetId>)` → 新增。使用 `FPrimaryAssetId` 存储因果链，天然兼容所有 `UPrimaryDataAsset` 子类（`UTcsStateDefinitionAsset`、`UTcsAttributeModifierDefinitionAsset`、未来的 SkillDefinitionAsset）
...
if (ParentSourceHandle.IsValid())
{
    ...
    NewChain.Add(ParentStateDefAsset->GetPrimaryAssetId());
}
...
- `UTcsStateDefinitionAsset` 不是 DataTable 行，无法用 `FDataTableRowHandle` 表示
...
| `Public/State/TcsState.h` | StateInstance 定义 |
| `Private/State/TcsState.cpp` | StateInstance 实现 |
