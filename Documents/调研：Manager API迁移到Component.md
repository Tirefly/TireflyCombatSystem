...
**实现**：`GetAttributeDefinition` 和 `GetModifierDefinition` 从 `AttributeDefinitions` / `AttributeModifierDefinitions` 中查找并返回，逻辑从现有 `AddAttribute` 和 `CreateAttributeModifier` 中提取。
...
- `GetAttributeDefinition` / `GetModifierDefinition`（新增 public）
...
| `bool EvaluateAndApplyStateParameters(...)` | `virtual bool EvaluateAndApplyStateParameters(const UTcsStateDefinition*, AActor* Instigator, UTcsStateInstance*, TArray<FName>&)` | virtual（删除 Owner 参数） |
...
- `GetStateDefinition` / `GetStateDefinitionByTag` / `GetStateSlotDefinition` / `GetStateSlotDefinitionByTag` / `GetAllStateDefNames`
...
### B-9. 更新 TcsStateInstance.cpp 中的 SetStackCount 调用路径

**文件**：`Private/State/TcsStateInstance.cpp` `SetStackCount()` 方法
...
这两个方法中引用了 `StateMgr->GetStateSlotDefinitionByTag`。迁移后 StateMgr 仍然可用（Component 持有缓存指针），调用方式不变。
...
| `Private/State/TcsStateInstance.cpp` | SetStackCount 调用路径改为通过 Component |
